...
# Global override: when set, every fetch ignores whatever `proxy` argument it
# was called with (including proxy="webshare_proxy" hardcoded in per-site
# discovery/parser classes) and goes direct. Off by default — only the
# no-proxy batch drivers turn this on.
_DISABLE_PROXY = False


def disable_proxy(flag: bool = True) -> None:
    global _DISABLE_PROXY
    _DISABLE_PROXY = bool(flag)
...
class sdfFetch:
    ...
    def get_page_content_hash(
        ...
        if not _DISABLE_PROXY and proxy == "webshare_proxy" and webshare_proxy:
