...
def run(project: str, site: str, max_depth: int = 1, limit: int = 10) -> None:
    ...
    # ── Depth 0: pagination ─────────────────────────────────────────────────
    # NOTE: per the production schema (see sdf_module/url_discovery.py
    # get_urls_by_depth), depth0 only holds seed_url; the method that turns
    # depth0 seeds into pagination URLs is declared under depth1, and the
    # method that turns those into product/article URLs is declared under
    # depth2. Read method names from the level that actually defines them.
    depth1_cfg = config.get("depth1", {})
    d0_method = depth1_cfg.get("method_name")
    ...
    depth2_cfg = config.get("depth2", {})
    d1_method = depth2_cfg.get("method_name")
    if not d1_method:
        return
