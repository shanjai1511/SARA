...
class DaMilanoComCommerceCrawl():
    ...
    def get_list_price(page_doc, inhash):
        ...
        elems = page_doc.xpath(
            "//*[contains(@class,'compare') or contains(@class,'was-price')][contains(@class,'price')]//text()"
        )
    ...
    def get_selling_price(page_doc, inhash):
        # `.product__price` is the current product's own price block; the old
        # generic `.price` match picked up unrelated related-product/upsell
        # prices earlier in the DOM, so every row got the same wrong value.
        elems = page_doc.xpath(
