...
class XyxxComCommerceCrawl():
    ...
    def get_list_price(page_doc, inhash):
        # Shopify "compare at" price only renders when the item is on sale.
        elems = page_doc.xpath(
            "//*[contains(concat(' ', normalize-space(@class), ' '), ' product__price--compare ')]//text()"
        )
    ...
    def get_selling_price(page_doc, inhash):
        # `.product__price` (exact class) is the current product's own price
        # block; the old generic `.price` match picked up unrelated
        # related-product/upsell grid prices earlier in the DOM, so every row
        # got the same wrong value.
        elems = page_doc.xpath(
            "//*[contains(concat(' ', normalize-space(@class), ' '), ' product__price ')]//text()"
        )
        val = elems[0].strip() if elems else None
        if val:
            val = ''.join(c for c in val if c.isdigit())
