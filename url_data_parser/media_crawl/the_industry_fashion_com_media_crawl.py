...
class TheIndustryFashionComMediaCrawl():
    ...
    def get_article_title(page_doc, inhash):
        ...
        return _re.sub(r'\s*[-|]\s*TheIndustry\.fashion\s*$', '', title, flags=_re.IGNORECASE).strip()
    ...
    def get_article_content(page_doc, inhash):
        elems = page_doc.xpath("//div[contains(@class,'story-content')]//p//text()")
        if not elems:
            elems = page_doc.xpath("//div[contains(@class,'entry-content')]//p//text() | //div[contains(@class,'post-content')]//p//text()")
