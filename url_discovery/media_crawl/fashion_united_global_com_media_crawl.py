...
# fashionunited.com uses ?page=N querystring pagination
    ...
    def get_product_url(self, url, depth, current_depth_level):
        ...
        try:
            ...
            for link in links:
                ...
                if "fashionunited.com" not in parsed.netloc:
                    continue
                path = parsed.path.lower()
                # FashionUnited articles: /news/topic/headline-20231201id/
                if "/news/" not in path:
                    continue
                parts = [p for p in path.strip("/").split("/") if p]
                # /news/topic/slug — need at least 3 segments
                if len(parts) < 3:
                    continue
                if full in seen:
                    continue
